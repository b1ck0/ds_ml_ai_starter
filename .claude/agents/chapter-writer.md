...
4. **Run the gate** and make it pass:
   ...
   - **render check** — `python .claude/hooks/check_markdown_render.py <the-chapter>.md` (the book is
     read on GitHub, which processes Markdown over `$…$`/`$$…$$` *before* MathJax and corrupts it).
     Fix every hit. Golden rules: **write every DISPLAYED equation as a ```math fenced block, never
     `$$…$$`** — the fence is the only venue GitHub doesn't Markdown-process, so `\text{one\_hot}`,
     `\begin{aligned}…\\…`, and multi-line math all survive there and break in `$$`. Keep inline `$…$`
     simple and on ONE line (no `\\`/matrices, no `<` before a letter — write `\lt`; no `\_ \# \{ \}`
     escapes). Keep Mermaid node labels containing `(`/`)` in `"double quotes"`.
