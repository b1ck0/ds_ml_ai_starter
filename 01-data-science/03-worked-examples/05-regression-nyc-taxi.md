...
### 3.1 R² as "beat the dumbest model"

MSE, RMSE, and MAE all answer "how many dollars off was I" — but that number is hard to judge in
isolation. Is an average miss of $2 good? It depends entirely on how hard the problem is. R²
answers a different, more useful question: **compared to the laziest possible model — one that
ignores every feature and always predicts the average fare — how much of the error did your model
actually remove?**

Picture two ways of drawing an "error line" from every point in a scatter plot:

- **Draw each error against the fitted line** — the line your model actually learned. The lengths
  of those error segments, squared and summed, is $SSE=\sum_i (y_i-\hat y_i)^2$: **"how wrong the
  model is."** ($\hat y_i$ = the model's prediction for point $i$.)
- **Draw each error against a flat line at the average** — $\hat y=\bar y$, the dumbest model
  there is: it doesn't look at a single feature, it just always answers "the average." Those
  segments, squared and summed, is $SST=\sum_i (y_i-\bar y)^2$: **"how wrong the dumbest model
  is."** ($\bar y$ = the average of $y$ across every row — read "y-bar.")

The figure below draws both, side by side, on the same ten synthetic points (not the taxi data —
small enough here that you can count every segment by eye):

![Two panels: residuals to the fitted line (SSE) on the left, residuals to the flat mean (SST) on the right, same ten points](artefacts/r2_sse_vs_sst.png)

Left panel: the blue fitted line, with a short red segment from each point down to that line —
that's SSE. Right panel: the *same points*, but now measured against the flat orange line at
$\bar y$ — the segments are visibly longer, because a flat line can't lean into the trend the way
the fitted line does. That's SST. Generated by
[`code/r2_intuition.py`](code/r2_intuition.py); running it prints the exact SSE, SST, and R² for
that dataset and cross-checks the hand-computed number against `sklearn.metrics.r2_score` on the
same points.

```mermaid
flowchart LR
    SST["SST: total variance in the data<br/>(how wrong the dumbest model is)"] --> SPLIT{split by the fitted model}
    SPLIT -->|"explained by the model"| EXP["SST − SSE<br/>(the trend the model captured)"]
    SPLIT -->|"left over"| SSE["SSE: residual error<br/>(how wrong the model still is)"]
    EXP --> R2["R² = (SST − SSE) / SST<br/>= 1 − SSE/SST<br/>(fraction of variance explained)"]
```

Now the formula reads as a sentence you already understand:

$$R^2 = 1-\frac{SSE}{SST}$$

$SSE/SST$ is "the fraction of the dumbest model's error your model still has left over." Subtract
that fraction from 1 and you get "the fraction of the dumbest model's error your model got rid
of" — which is exactly the same thing as **the fraction of the data's variance your model
explains**, because $SST$ is just the variance of $y$ scaled up by the row count:
$SST=\mathrm{Var}(Y)\cdot n$ (population variance, i.e. the same "divide by $n$" convention
`numpy`'s default `np.var` uses, not the "divide by $n-1$" sample-variance convention)
([source: Wikipedia, "Coefficient of determination"](https://en.wikipedia.org/wiki/Coefficient_of_determination),
checked 2026-09-03 — confirms $R^2=1-SS_{res}/SS_{tot}$ and that $SS_{tot}/n$ is the population
variance of $y$; cross-checked against
[sklearn's `r2_score` reference](https://scikit-learn.org/stable/modules/generated/sklearn.metrics.r2_score.html),
checked 2026-09-03, and verified numerically in `r2_intuition.py`'s
`assert abs(sst - np.var(y, ddof=0) * len(y)) < 1e-8`).

Two landmarks worth memorizing:

- **$R^2=0$** means $SSE=SST$ — your model's errors are exactly as large as the dumbest model's.
  You built a whole pipeline to tie with "always guess the average."
- **$R^2=1$** means $SSE=0$ — every prediction lands exactly on the true value.

Tie this back to §4's real numbers: the `plus_distance` stage's `LinearRegression` scores
**R² = 0.9793** — it explains about **97.9% of the variance in taxi fares**, leaving roughly 2.1%
of that variance as leftover error. That leftover 2.1% is not a mystery number — it's precisely
what §8's residual histogram, residual-vs-fitted scatter, and parity plot go looking through, row
by row, to make sure it isn't hiding a systematic bias rather than honest noise.
